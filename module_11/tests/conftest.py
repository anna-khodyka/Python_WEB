...
def _init_db():
    ...
    record = Record(name='test name', birthday=None,
                    address='test address', email='test@test.ts', tags="test tag")
    record.phones.append(Phone(phone_value="+380661112233"))
    db.session.add(record)
    db.session.add(Note(note_tags='test tag',
                        note_text='test text', created_at=None))

    for _ in range(10):
        record = Record(name=fake.name(), birthday=None,
        ...
        db.session.add(Note(note_tags=fake.text(max_nb_chars=10),
                            note_text=fake.text(max_nb_chars=30), created_at=None))
    db.session.commit()
