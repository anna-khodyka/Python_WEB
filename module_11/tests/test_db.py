...
def test_test_record_exists(app):
    with app.app_context():
        db = get_db()
        test_record = db.session.query(Record).filter_by(name="test name").one()
        assert test_record
...
def test_test_phone_exists(app):
    with app.app_context():
        db = get_db()
        test_phone = (
            db.session.query(Phone).filter_by(phone_value="+380661112233").one()
        )
        assert test_phone
...
def test_test_note_exists(app):
    with app.app_context():
        db = get_db()
        test_note = db.session.query(Note).filter_by(note_tags="test tag").one()
        assert test_note
