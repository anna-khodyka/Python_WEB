...
def test_add(client, app):
    ...
    response = client.post(
        '/notes/add', data={'note_tags': 'to do list', 'note_text': 'do nothing', 'created_at': '27.10.2021'}, follow_redirects=True
    )
...
def test_add_with_bad_created_at(client, app):
    response = client.post(
        '/notes/add', data={'note_tags': 'wrong note', 'note_text': 'wrong text', 'created_at': '27.102021'}, follow_redirects=True
    )
    assert response.status_code == 200
...
def test_add_with_empty_text(client, app):
    response = client.post(
        '/notes/add', data={'note_tags': 'wrong note', 'note_text': '', 'created_at': '27.10.2021'}, follow_redirects=True
    )
    assert response.status_code == 200
...
def test_delete(client, app):
    response = client.post(
        "/notes/12/delete", follow_redirects=True)
    assert response.status_code == 200
    print(response.data)
...
def test_edit(client, app):
    ...
    response = client.post(
        '/notes/1/edit', data={'note_tags': 'upd tag', 'note_text': 'test text', 'created_at': '1980-06-12'}, follow_redirects=True
    )
    ...
    response = client.post(
        '/notes/1/edit', data={'note_tags': 'upd tag', 'note_text': 'test text', 'created_at': '12.06.1975'}, follow_redirects=True
    )
    assert b'upd tag' in response.data
    assert response.status_code == 200
    assert b'Edit' in response.data
...
def test_edit_with_empty_text(client, app):
    response = client.post(
        '/notes/1/edit', data={'note_tags': 'upd tag', 'note_text': '', 'created_at': '1980-06-12'}, follow_redirects=True
    )
    assert response.status_code == 200
...
def test_edit_with_wrong_created_at(client, app):
    response = client.post(
        '/notes/1/edit', data={'note_tags': 'upd tag', 'note_text': 'test text', 'created_at': '1980-0612'}, follow_redirects=True
    )
    assert response.status_code == 200
    assert b"Wrong date." in response.data
