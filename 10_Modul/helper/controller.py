...
class Controller:
        ...
        while True:
            ...
            if decision in YES_DECISION:
                ...
                if self.model.book.validate_birthday(birthday):
                    # birthday хранится в формате строка !
    ...
    # @error_handler
    def change(self):
        # определяем имя контакта что надо изменить
        self.view.notify_of_message(100*'_')
        ...
        if records_list:
            ...
            self.view.notify_of_message(
                f"I've found the following notes with this word")
            self.show_find(records_list)
            # ввод имени для редактирования
            edited_name = self.view.input_name(
                message="Input the fuul name of record you want to edit:")
        else:
        ...
        result = executing_command[changed_field](edited_name, new_field_value)
    ...
    def delete(self):
        ...
        # находим список документов book, имя которых содержит keyword
        records_list = self.model.book.find_value(keyword)

        if records_list:
            # если поиск дал результат
            self.view.notify_of_message(
                f"I've found the following records with this Name")
            self.show_find(records_list)
            # ввод id для удаления
            del_name = self.view.input_name(
                message="Input full name of record that you want to delete:")
            result = self.model.book.remove_record(del_name)
            self.view.notify_of_message("Thecontact deletes to DB")
        else:
            self.view.notify_of_message(
    ...
    def show_find(self, records_list):
        ...
        iter = self.model.book.iterator(records_list)
        for i in iter:
    ...
    def edit_note(self):
        ...
        old_note_text = self.model.notes_book.get_note_text_for_editing(
            hashtag)
